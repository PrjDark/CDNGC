		...
		public static ContentReturn CheckNetworkStatus() {
			try {
				...
				if (GameCommon.DNetMarker[0] == "d-Network") {
					...
					GameCommon.DNetCInfo = new DNet("http://CDNGC.network.xprj.net/Information/Circle.txt").GetStrings();
					...
					DNet dNet2 = new DNet("http://CDNGC.update.network.xprj.net/" + GameCommon.Version.GetNet() + ".txt");
					if (dNet2.Status <= 350) {
						GameCommon.DNetNewVer = dNet2.GetStrings();
					} else {
						dNet2 = new DNet(string.Concat(new string[]
						{
							"http://CDNGC.update.network.xprj.net/",
						...
						if (dNet2.Status <= 350) {
							...
							dNet2 = new DNet("http://CDNGC.update.network.xprj.net/" + GameCommon.Version.APPID + ".txt");
							if (dNet2.Status <= 350) {
								GameCommon.DNetNewVer = dNet2.GetStrings();
							} else {
